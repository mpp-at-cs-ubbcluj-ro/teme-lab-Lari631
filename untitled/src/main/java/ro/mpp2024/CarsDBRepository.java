...
public class CarsDBRepository implements CarRepository{
    ...
    public List<Car> findBetweenYears(int min, int max) {
        ...
        Connection con = dbUtils.getConnection();
        List<Car> cars = new ArrayList<>();
        try (PreparedStatement preparedStatement = con.prepareStatement("select * from cars where year in (?,?)")) {
    ...
    public void update(Integer integer, Car elem) {
        logger.traceEntry("saving task {}",elem);
        Connection con=dbUtils.getConnection();
        try(PreparedStatement preparedStatement=con.prepareStatement("update cars set manufacturer=? where id=?")){
            preparedStatement.setString(1, elem.getManufacturer());
